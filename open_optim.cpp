...
public:
    ...
    //sub to husky goal topic (becomes sub to cf topic)
    ...
    void solveAndPublishCmdVel(ros::Publisher& publisher, ros::Publisher& hgoal_publisher)
    {
        ...
        double hgoal_x, hgoal_y = 0;
        
        for (int i=0; i<NX; i++) {
            current_par[i] = current_pos[i];        //for each time step add husky pose and cf goal to params
...
int main(int argc, char** argv)
{
    ...
    private_nh.param("out_twist_name", out_twist, std::string("/husky_velocity_controller/cmd_vel"));   //pub vel command alias
    private_nh.param("in_odom_name", in_odometry, std::string("/odometry/filtered"));                   //sub husky pose alias
    ...
    //NEW: sub to cf goal
    ros::Subscriber command_trajectory_subscriber
        = private_nh.subscribe("/GoalPoseCF",
                               1,
                               &open_nmpc_controller::OptimizationEngineManager::commandPoseCallback,
                               &mng);
    ...
    //NEW: also pub to husky goal
    ros::Publisher pub_husky_goal = nh.advertise<nav_msgs::Odometry>("/goalPose", 1);
    
    ros::Rate loop_rate(rate);

    while (ros::ok()) {
        mng.solveAndPublishCmdVel(pub_twist_cmd, pub_husky_goal);   //NEW: pass vel publisher and husky goal publisher
        ros::spinOnce();
        loop_rate.sleep();
    }
